...
def get_coordinates():
    ...
    print(text_content)
    return render_template('res.html', txt_content=text_content)

def cutAtVector(filename, x, y):
    matplotlib.use('agg')
    cut(x, y, initDevice(), filename)
    # Vectorize

    vtracer.convert_image_to_svg_py(image_path='static/mask_0.png', 
                                    out_path='static/mask_0_vectorized.svg',
                                    colormode = 'binary',
                                    # corner_threshold = 20,
                                    # splice_threshold = 20,
                                    path_precision = 3,
...
def cutAt(filename, x, y):
    matplotlib.use('agg')
    cut(x, y, initDevice(), filename)
    return 'static/mask_0.png'
...
def save_masks(image, masks, scores, point_coords=None, box_coords=None, input_labels=None, borders=True):
    ...
    for i, (mask, score) in enumerate(zip(masks, scores)):
        ...
        show_mask(mask, ax, borders=False)
